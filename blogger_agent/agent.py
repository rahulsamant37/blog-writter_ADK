...
from .tools import analyze_codebase, readability_scorer, save_blog_post_to_file

# --- AGENT DEFINITIONS ---

interactive_blogger_agent = LlmAgent(
    ...
    6.  **Readability (Optional):** If the user asks for readability feedback, use the `readability_scorer` tool to compute Flesch-Kincaid scores, flag complex sentences, and analyze paragraph length/structure.
    7.  **Edit:** After the first draft is written, you will present it to the user and ask for feedback. You will then revise the blog post based on the feedback. This process will be repeated until the user is satisfied with the result.
    8.  **Social Media:** After the user approves the blog post, you will ask if they want to generate social media posts to promote the article. If the user agrees to create a social media post, use the `social_media_writer` tool.
    9.  **Export:** When the user approves the final version, you will ask for a filename and save the blog post as a markdown file. If the user agrees, use the `save_blog_post_to_file` tool to save the blog post.
    ...
    tools=[
        ...
        FunctionTool(readability_scorer),
    ],
